...
function createData(titre, auteur, disponible, location, etat) {
	return { titre, auteur, disponible, location, etat };
}
	...
	useEffect(() => {
		...
		function makeRows(books) {
			const rows = books.map((book) => {
				...
				const state = (
					<div>
						<Link href={`/books/edit/${book._id}`} className={styles.edit}>
							<EditIcon />
						</Link>
						<Link href={`/books/remove/${book._id}`} className={styles.remove}>
							<DeleteOutlineIcon />
						</Link>
					</div>
				);
				const location = book.self_service_id[0]._id;
				return createData(book.title, book.author, availableSign, location, state);
			});
			setRows(rows);
		}
